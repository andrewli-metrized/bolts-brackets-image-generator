...
// Changed OutfitLayer to AssetItem to match types.ts
import { AssetItem } from '../types';
import { Trash2Icon, PlusIcon } from './icons';

interface OutfitStackProps {
  // Changed outfitHistory type to AssetItem[]
  outfitHistory: AssetItem[];
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="space-y-2 flex-grow">
        {outfitHistory.map((layer, index) => (
          <div
            // Updated property access from layer.garment to layer
            key={layer.id || 'base'}
          ...
          >
            <div className="flex items-center overflow-hidden">
                ...
                {layer.url && (
                    <img src={layer.url} alt={layer.name} className="flex-shrink-0 w-12 h-12 object-cover rounded-md mr-3" />
                )}
                <span className="font-semibold text-gray-800 truncate" title={layer.name}>
                  {layer.name || 'Base Model'}
                </span>
            </div>
