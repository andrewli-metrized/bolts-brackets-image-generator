...
// Changed defaultWardrobe to defaultAssets to match wardrobe.ts
    ...
    <AnimatePresence>
        {isOpen && (
            <motion.div
                ...
                exit={{ opacity: 0 }}
                onClick={onClose}
                className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 flex items-center justify-center p-4 font-sans"
            >
                <motion.div
                ...
                >
                    <div className="flex items-center justify-between p-4 border-b">
                        <h2 className="text-2xl font-sans font-bold tracking-tight text-gray-800">Add Garment</h2>
                        ...
                        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4">
                            ...
                            return (
                                ...
                                <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                    <p className="text-white text-xs font-bold text-center p-1 font-sans">{item.name}</p>
                                </div>
                                {isActive && (
                            ...
                            <label htmlFor="custom-garment-upload" className={`relative aspect-square border-2 border-dashed rounded-lg flex flex-col items-center justify-center text-gray-500 transition-colors ${isLoading ? 'cursor-not-allowed bg-gray-100' : 'hover:border-gray-400 hover:text-gray-600 cursor-pointer'}`}>
                                ...
                                <span className="text-xs text-center font-sans">Upload</span>
